<template>
  <a-layout id="components-layout-demo-responsive">
    ...
    >
      <div class="logo" />
      <a-menu theme="dark" mode="inline" :openKeys="openKeys" @openChange	="openChange	"  @select="select" :selectedKeys="[$route.path]" >
        <template v-for="item in menu" >
          ...
          <a-sub-menu v-else :key="item.url">
            <span slot="title"><a-icon type="appstore" /><span>{{item.name}}</span></span>
        ...
        <a-layout  style="height:100%">
          <a-layout-header :style="{ background: '#fff', padding: 0 }">
             <a-dropdown :trigger="['contextmenu']">
              <a-tabs @contextmenu.native="contextmenu" v-model="$store.state.activePage" type="editable-card" @tabClick="tabClick" @edit="editPage" hideAdd>
                <a-tab-pane v-for="pane in $store.state.pageList"  :tab="pane.name" :key="pane.fullPath"  :closable="true">
                </a-tab-pane>
              </a-tabs>
              <a-menu slot="overlay">
                <a-menu-item key="1">
